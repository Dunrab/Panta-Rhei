...
public sealed partial class SupermatterSystem
{
 ...
 // Keep in mind we are only adding this temperature to (efficiency)% of the one tile the rock is on.
    ...
    public DelamType ChooseDelamType(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (mix is { })
        {
            ...
            if (_config.GetCVar(EECVars.SupermatterDoSingulooseDelam)
                && moles >= sm.MolePenaltyThreshold * _config.GetCVar(EECVars.SupermatterSingulooseMolesModifier))
                return DelamType.Singulo;
        }

        if (_config.GetCVar(EECVars.SupermatterDoTeslooseDelam)
            && sm.Power >= sm.PowerPenaltyThreshold * _config.GetCVar(EECVars.SupermatterTesloosePowerModifier))
    ...
    private void HandleAccent(EntityUid uid, SupermatterComponent sm)
    {
        ...
        RaiseLocalEvent(uid, sm); // was (uid, ev)
    }
}
