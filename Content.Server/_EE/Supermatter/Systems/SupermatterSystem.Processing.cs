...
public sealed partial class SupermatterSystem
{
    ...
    private void ProcessAtmos(EntityUid uid, SupermatterComponent sm, float frameTime)
    {
        ...
        sm.MoleHeatPenaltyThreshold = (float)Math.Max(sm.GasStorage.TotalMoles / _config.GetCVar(EECVars.SupermatterMoleHeatPenalty), 0.25);

        // Ramps up or down in increments of 0.02 up to the proportion of CO2
        // Given infinite time, powerloss_dynamic_scaling = co2comp
        // Some value from 0-1
        if (sm.GasStorage.TotalMoles > _config.GetCVar(EECVars.SupermatterPowerlossInhibitionMoleThreshold) && // if there are more than 20 mols,
            gasComposition.GetMoles(Gas.CarbonDioxide) > _config.GetCVar(EECVars.SupermatterPowerlossInhibitionGasThreshold)) // and more than 20% co2
        {
        ...
        sm.PowerlossInhibitor = Math.Clamp(
            1 - sm.PowerlossDynamicScaling * Math.Clamp(sm.GasStorage.TotalMoles / _config.GetCVar(EECVars.SupermatterPowerlossInhibitionMoleBoostThreshold), 1f, 1.5f),
            0f, 1f);
    ...
    private void HandleDamage(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.GasStorage is { })
        {
            ...
            var moleDamage = Math.Max(sm.GasStorage.TotalMoles - _config.GetCVar(EECVars.SupermatterMolePenaltyThreshold), 0f) / 80 * sm.DamageIncreaseMultiplier;
            totalDamage += moleDamage;
        }

        // Healing damage
        if (sm.GasMixture is { } && sm.GasStorage is { } && sm.GasStorage.TotalMoles < _config.GetCVar(EECVars.SupermatterMolePenaltyThreshold))
        {
        ...
        if (TryComp<AppearanceComponent>(uid, out var appearance))
        {
            ...
            if (totalDamage > 0)
            {
                ...
                {
                    Shared._EE.Supermatter.Components.SupermatterStatusType.Delaminating => SupermatterCrystalState.GlowDelam,
                    >= Shared._EE.Supermatter.Components.SupermatterStatusType.Emergency => SupermatterCrystalState.GlowEmergency,
    ...
    private void AnnounceCoreDamage(EntityUid uid, SupermatterComponent sm)
    {
        ...
        // We're safe
        if (sm.Damage < sm.DamageArchived && sm.Status >= Shared._EE.Supermatter.Components.SupermatterStatusType.Warning)
        {
            message = Loc.GetString("supermatter-healing", ("integrity", integrity));

            if (sm.Status >= Shared._EE.Supermatter.Components.SupermatterStatusType.Emergency)
                global = true;
    ...
    public DelamType ChooseDelamType(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.GasStorage is { })
        {
            if (_config.GetCVar(EECVars.SupermatterDoSingulooseDelam)
                && sm.GasStorage.TotalMoles >= _config.GetCVar(EECVars.SupermatterMolePenaltyThreshold) * _config.GetCVar(EECVars.SupermatterSingulooseMolesModifier))
    ...
    private void HandleStatus(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.Status != currentStatus && HasComp<DeviceLinkSourceComponent>(uid))
        {
            ...
            {
                Shared._EE.Supermatter.Components.SupermatterStatusType.Normal => sm.PortNormal,
                Shared._EE.Supermatter.Components.SupermatterStatusType.Caution => sm.PortCaution,
                Shared._EE.Supermatter.Components.SupermatterStatusType.Warning => sm.PortWarning,
                Shared._EE.Supermatter.Components.SupermatterStatusType.Danger => sm.PortDanger,
                Shared._EE.Supermatter.Components.SupermatterStatusType.Emergency => sm.PortEmergency,
                Shared._EE.Supermatter.Components.SupermatterStatusType.Delaminating => sm.PortDelaminating,
                _ => sm.PortInactive
            };
        ...
        if (sm.Damage < sm.DamageArchived && currentStatus != Shared._EE.Supermatter.Components.SupermatterStatusType.Delaminating)
        {
        ...
        {
            Shared._EE.Supermatter.Components.SupermatterStatusType.Warning => sm.StatusWarningSound,
            Shared._EE.Supermatter.Components.SupermatterStatusType.Danger => sm.StatusDangerSound,
            Shared._EE.Supermatter.Components.SupermatterStatusType.Emergency => sm.StatusEmergencySound,
            Shared._EE.Supermatter.Components.SupermatterStatusType.Delaminating => sm.StatusDelamSound,
            _ => sm.StatusSilentSound
        };

        if (currentStatus == Shared._EE.Supermatter.Components.SupermatterStatusType.Warning)
    ...
    private void HandleSoundLoop(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.Status >= Shared._EE.Supermatter.Components.SupermatterStatusType.Danger && sm.CurrentSoundLoop != sm.DelamLoopSound)
            sm.CurrentSoundLoop = sm.DelamLoopSound;

        else if (sm.Status < Shared._EE.Supermatter.Components.SupermatterStatusType.Danger && sm.CurrentSoundLoop != sm.CalmLoopSound)
    ...
    private void HandleAccent(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.Status >= Shared._EE.Supermatter.Components.SupermatterStatusType.Danger)
