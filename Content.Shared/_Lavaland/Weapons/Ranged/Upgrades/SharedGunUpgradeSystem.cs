...
public abstract partial class SharedGunUpgradeSystem : EntitySystem
{
    ...
    */
    ...
    private void OnUpgradeExamine(Entity<GunUpgradeLavalandComponent> ent, ref ExaminedEvent args)
    {
    ...
    {
        if (!TryComp<GunUpgradeLavalandComponent>(args.Item, out var upgradeComp)
    ...
    public HashSet<Entity<GunUpgradeLavalandComponent>> GetCurrentUpgrades(Entity<UpgradeableWeaponComponent> ent, ItemSlotsComponent? itemSlots = null)
    {
        if (!Resolve(ent, ref itemSlots))
            return [];

        var upgrades = new HashSet<Entity<GunUpgradeLavalandComponent>>();

        foreach (var itemSlot in itemSlots.Slots.Values)
        {
            if (itemSlot is { HasItem: true, Item: { } item }
                && TryComp<GunUpgradeLavalandComponent>(item, out var upgradeComp))
